...
async def check_position(pos,db,dex,sol_usd):
    ...
    if reason:
        pnl=pnl_pct(pos.entry_price,pair.price_usd); db.close_position(pos.id,pair.price_usd,reason,pnl,sol_usd)
...
async def run():
    ...
    try:
        ...
        # Make a baseline snapshot immediately so the new history is protected.
        db.backup()
        while True:
            try:
                sol_usd=await dex.sol_price_usd()
                ...
                # Rotate a consistent SQLite snapshot every ~5 minutes and keep 12 copies.
                ...
                log.exception('loop error; continuing'); await asyncio.sleep(min(30,max(2,settings.poll_seconds*2)))
    finally:
