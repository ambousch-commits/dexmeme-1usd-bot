...
class Settings:
    ...
    min_buys:int=int(os.getenv('MIN_BUYS','100')); min_liquidity_usd:float=float(os.getenv('MIN_LIQUIDITY_USD','10000')); max_open_positions:int=int(os.getenv('MAX_OPEN_POSITIONS','20'))
    stop_loss_pct:float=float(os.getenv('STOP_LOSS_PCT','-5')); take_profit_min_pct:float=float(os.getenv('TAKE_PROFIT_MIN_PCT','8')); take_profit_max_pct:float=float(os.getenv('TAKE_PROFIT_MAX_PCT','15'))
    ...
    min_buys_1h:int=int(os.getenv('MIN_BUYS_1H','10')); buy_sell_ratio:float=float(os.getenv('BUY_SELL_RATIO','1.25')); min_price_change_1h_pct:float=float(os.getenv('MIN_PRICE_CHANGE_1H_PCT','1'))
