...
class Settings:
    ...
    min_buys:int=int(os.getenv('MIN_BUYS','100')); min_liquidity_usd:float=float(os.getenv('MIN_LIQUIDITY_USD','1200')); max_liquidity_usd:float=float(os.getenv('MAX_LIQUIDITY_USD','2500')); max_open_positions:int=int(os.getenv('MAX_OPEN_POSITIONS','20'))
    ...
    reentry_cooldown_minutes:int=int(os.getenv('REENTRY_COOLDOWN_MINUTES','60'))
    emergency_liquidity_drop_pct:float=float(os.getenv('EMERGENCY_LIQUIDITY_DROP_PCT','30'))
    emergency_sell_buy_ratio:float=float(os.getenv('EMERGENCY_SELL_BUY_RATIO','1.25'))
    emergency_price_drop_pct:float=float(os.getenv('EMERGENCY_PRICE_DROP_PCT','8'))
    ...
    def validate(self):
        ...
        if self.min_liquidity_usd<=0 or self.max_liquidity_usd<self.min_liquidity_usd: raise ValueError('invalid liquidity range')
