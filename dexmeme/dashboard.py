    ...
    if os.path.isdir('/data'):
        for name in sorted(os.listdir('/data')):
            if name.endswith(('.sqlite3','.db')):
                p=os.path.abspath(os.path.join('/data',name))
...
def active_db():
    ...
    for path in db_candidates():
        ...
        try:
            db=Database(path)
            count=int(db.conn.execute('SELECT COUNT(*) FROM positions').fetchone()[0])
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if path=='/health':
            self._json({'ok':True,'mode':'paper','configured_db':settings.db_path,'db_candidates':db_candidates()}); return
        if path=='/api/stats':
            best=active_db()
            if best is None:
        ...
        if path=='/api/trades.csv':
            ...
            self.send_response(200); self.send_header('Content-Type','text/csv; charset=utf-8'); self.send_header('Content-Disposition','attachment; filename="dexmeme-1usd-trades.csv"'); self.send_header('Cache-Control','no-store'); self.end_headers(); self.wfile.write(body); return
